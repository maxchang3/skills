...
This guide explains how an extension can provide AI models to Raycast.
...
Raycast calls `getModels` to discover the models your extension currently provides — during user-initiated setup, periodically in the background after the user has allowed access and completed setup, and whenever you call [`AI.refreshModels`](https://developers.raycast.com/ai/pages/RntXmm9tJ3rw5ho48kFJ#ai.refreshmodels) for an enabled provider.
...
When the set of available models changes outside a regular refresh — for example after the user downloads a new local model through one of your commands — call [`AI.refreshModels`](https://developers.raycast.com/ai/pages/RntXmm9tJ3rw5ho48kFJ#ai.refreshmodels) to make Raycast re-run `getModels`:
...
Your extension can target its own provided models from [`AI.ask`](https://developers.raycast.com/ai/pages/RntXmm9tJ3rw5ho48kFJ#ai.ask) by passing the local model `id`:
